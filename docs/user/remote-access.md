...
Use this when you want to connect to a Trunk server from another device such as a phone, tablet, or separate desktop app.
...
Use this when you want the desktop app to start or reuse Trunk on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. Trunk uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Trunk first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Trunk uses:
...
Hosted pairing does not proxy traffic through Trunk. The browser still connects directly to the backend URL in the pairing link.
