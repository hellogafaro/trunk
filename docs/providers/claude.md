...
This guide is for people who want to use more than one Claude setup in Trunk.
...
In Trunk Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means Trunk uses your normal home directory.
...
Then add another Claude provider in Trunk:
...
Trunk only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Trunk keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in Trunk Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Trunk stores the value as a server secret and does not
...
Trunk does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in Trunk.
