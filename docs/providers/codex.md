...
This guide is for people who want to use more than one Codex account in Trunk.
...
In Trunk Settings, name it something obvious:
...
In Trunk Settings, add another Codex provider:
...
Trunk shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Trunk treats it
