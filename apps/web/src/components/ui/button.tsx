...
const buttonVariants = cva(
  "[&_svg]:-mx-0.5 relative inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-sm border font-medium text-sm outline-none transition-shadow before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-sm)-1px)] focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-64 [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-5 md:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    defaultVariants: {
      size: "default",
      variant: "default",
    ...
    variants: {
      size: {
        default: "h-11 px-[calc(--spacing(3)-1px)] md:h-8",
        icon: "size-11 md:size-8",
        "icon-lg": "size-12 md:size-9",
        "icon-sm": "size-11 md:size-7",
        "icon-xl":
          "size-12 md:size-10 [&_svg:not([class*='size-'])]:size-5 md:[&_svg:not([class*='size-'])]:size-4.5",
        "icon-xs":
          "size-11 rounded-sm before:rounded-[calc(var(--radius-sm)-1px)] md:size-6 not-in-data-[slot=input-group]:[&_svg:not([class*='size-'])]:size-4 md:not-in-data-[slot=input-group]:[&_svg:not([class*='size-'])]:size-3.5",
        lg: "h-12 px-[calc(--spacing(3.5)-1px)] md:h-9",
        sm: "h-11 gap-1.5 px-[calc(--spacing(2.5)-1px)] md:h-7",
        xl: "h-12 px-[calc(--spacing(4)-1px)] text-lg md:h-10 md:text-base [&_svg:not([class*='size-'])]:size-5 md:[&_svg:not([class*='size-'])]:size-4.5",
        xs: "h-11 gap-1 rounded-sm px-[calc(--spacing(2)-1px)] text-sm before:rounded-[calc(var(--radius-sm)-1px)] md:h-6 [&_svg:not([class*='size-'])]:size-4 md:[&_svg:not([class*='size-'])]:size-3.5",
      },
      variant: {
        default:
