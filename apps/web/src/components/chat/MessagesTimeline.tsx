  ...
  if (rows.length === 0 && !isWorking) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-6 text-center text-muted-foreground/78">
        <Logo className="size-9 opacity-10" />
...
function WorkingTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "working" }> }) {
  return (
    <div className="py-0.5 pl-1.5">
      <div className="flex items-center gap-2 pt-1 text-xs text-muted-foreground/70 tabular-nums">
...
const WorkGroupSection = memo(function WorkGroupSection({
  ...
  return (
    <section className="-mx-1 space-y-0.5 px-1 py-0.5" aria-label={groupLabel}>
      {!onlyToolEntries && (
        <p className="px-0.5 pb-0.5 font-medium text-xs text-muted-foreground/65">{groupLabel}</p>
      )}
...
function WorkGroupToggleTimelineRow({
  ...
  return (
    <button
      type="button"
      className="flex w-full cursor-pointer items-center gap-1.5 rounded-md px-0.5 py-0.5 text-left text-sm leading-5 transition-colors duration-150 hover:bg-accent/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/70"
      aria-expanded={row.expanded}
      onClick={(event) => {
        const anchorElement =
...
const UserMessageBody = memo(function UserMessageBody(props: {
  ...
  if (reviewCommentSegments.some((segment) => segment.kind === "review-comment")) {
    return (
      <div className="space-y-3 text-base leading-relaxed text-foreground md:text-sm">
  ...
  if (props.terminalContexts.length > 0) {
    ...
    if (hasEmbeddedInlineLabels) {
      ...
      if (inlineNodes.length > 0) {
        ...
        return (
          <div className="whitespace-pre-wrap wrap-break-word text-base leading-relaxed text-foreground md:text-sm">
            {inlineNodes}
          </div>
        );
      }
