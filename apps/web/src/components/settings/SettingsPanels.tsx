...
const PROVIDER_SETTINGS: readonly InstallProviderSettings[] = [
  ...
  },
  {
    ...
    serverUrlDescription: "Leave blank to let Trunk spawn the server when needed",
...
function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in Trunk.",
    };
  }
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="General">
        <SettingsRow
          title="Theme"
          description="Choose how Trunk looks across the app."
          resetAction={
            theme !== "system" ? (
