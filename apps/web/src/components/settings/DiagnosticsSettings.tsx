      ...
      <div className="flex min-w-0 items-center gap-1.5 text-xs font-medium uppercase tracking-[0.08em] text-muted-foreground/70">
        ...
        {tooltip ? (
          <Tooltip>
            ...
            <TooltipPopup
              side="top"
              className="max-w-[min(300px,calc(100vw-2rem))] whitespace-normal text-left text-xs leading-relaxed text-wrap"
            >
              {tooltip}
            </TooltipPopup>
          </Tooltip>
  ...
  return (
    <div className={cn("min-w-0", className)}>
      ...
      {canExpand ? (
        <button
          type="button"
          className="mt-1 text-xs font-medium text-foreground/70 underline-offset-2 hover:text-foreground hover:underline"
  ...
  return (
    <div className="flex w-full min-w-0 max-w-full items-center gap-2">
      <Tooltip>
        ...
        <TooltipPopup
          side="top"
          className="max-w-[min(520px,calc(100vw-2rem))] break-all font-mono text-xs"
        >
          {traceId}
        </TooltipPopup>
      </Tooltip>
  ...
  return (
    ...
    >
      ...
      <Tooltip>
        ...
        <TooltipPopup
          side="top"
          className="max-w-[min(440px,calc(100vw-2rem))] whitespace-normal break-words text-left font-mono text-xs leading-relaxed text-wrap"
        >
          {process.command}
        </TooltipPopup>
      </Tooltip>
        ...
        <TooltipTrigger
          render={
            <button
              ...
              className="text-xs font-medium text-muted-foreground underline-offset-2 hover:text-foreground hover:underline disabled:pointer-events-none disabled:opacity-50"
        ...
        <TooltipTrigger
          render={
            <button
              type="button"
              disabled={isSignaling}
              className="text-xs font-medium text-destructive underline-offset-2 hover:underline disabled:pointer-events-none disabled:opacity-50"
    ...
    <div className="flex items-center rounded-md border border-border/60 p-0.5">
      {RESOURCE_HISTORY_WINDOWS.map((option) => (
        <button
          ...
          type="button"
          className={cn(
            "h-6 rounded-sm px-2 text-xs font-medium text-muted-foreground hover:text-foreground",
        ...
        if (!result.value.signaled) {
          ...
          if (isStaleProcessSignalMessage(message)) {
            toastManager.add({
              ...
              description:
                "The process is not a child of the Trunk server. It might already have exited.",
            });
            return;
          }
