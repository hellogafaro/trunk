  ...
  return (
    <Tooltip>
      ...
      <TooltipPopup side="top" className="max-w-72 whitespace-normal leading-relaxed">
        Trunk does not recognize this condition yet. It can still be saved, but it may not match
...
function WhenVariableSelect({
  ...
  return (
    <Select value={value} onValueChange={(nextValue) => nextValue && onChange(nextValue)}>
      ...
      >
        {options.map((option) => (
          <SelectItem key={option} value={option} className="min-h-7 w-full py-1 font-mono text-sm">
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        <Select value={operator} onValueChange={(value) => setOperator(value as BooleanOperator)}>
          ...
          >
            <SelectItem value="and" className="min-h-7 py-1 font-mono text-sm">
              and
            </SelectItem>
            <SelectItem value="or" className="min-h-7 py-1 font-mono text-sm">
              or
            </SelectItem>
          </SelectContent>
        </Select>
      ...
      <div className="space-y-1.5">
        <div className="relative">
          <Input
            ...
            className={cn(
              "h-7 rounded-md font-mono text-sm leading-7 sm:h-7 sm:leading-7",
    ...
    <div className="grid grid-cols-[minmax(190px,1.1fr)_minmax(220px,0.85fr)_minmax(210px,1fr)_60px] items-center px-4 py-1.5 text-sm even:bg-muted/15 hover:bg-accent/40">
      ...
      <div className="flex min-w-0 items-center gap-2 pr-4">
        {showPill ? (
          ...
          <Input
            ...
            className={cn(
              "h-7 w-44 rounded-md font-mono text-sm sm:h-7",
      ...
      <div className="pr-4">
        <Popover>
          <PopoverTrigger
            className={cn(
              "inline-flex h-7 w-full items-center justify-between gap-2 rounded-md border border-input bg-background px-2.5 text-left font-mono text-sm text-foreground shadow-xs/5 outline-none transition-colors hover:bg-accent focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/24",
        ...
        >
          ...
          >
            {commandOptions.map((command) => (
              <SelectItem key={command} value={command} className="min-h-7 w-full py-1 text-sm">
...
export function KeybindingsSettingsPanel() {
  ...
  return (
    <SettingsPageContainer className="max-w-5xl">
      ...
      >
        {!isElectron ? (
          <div className="flex items-start gap-2 border-b border-warning/20 bg-warning/5 px-3 py-2.5 text-sm leading-relaxed text-muted-foreground sm:px-4">
            <InfoIcon className="mt-0.5 size-3.5 shrink-0 text-warning" />
            <p>
              Some shortcuts may be claimed by the browser before Trunk sees them. Use the desktop
