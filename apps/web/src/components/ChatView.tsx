...
function ChatViewContent(props: ChatViewProps) {
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    const turnAttachmentsPromise = Promise.all(
      [
        ...composerImagesSnapshot.map((image) => ({ type: "image" as const, attachment: image })),
        ...composerFilesSnapshot.map((file) => ({ type: "file" as const, attachment: file })),
      ].map(async ({ type, attachment }) => ({
        type,
              ...
              {showScrollToBottom && (
                ...
                >
                  <button
                    ...
                    className="pointer-events-auto flex min-h-11 items-center gap-1.5 rounded-full border border-border/60 bg-card px-3 py-1 text-muted-foreground text-xs shadow-sm transition-colors hover:border-border hover:text-foreground hover:cursor-pointer md:min-h-0"
                  >
